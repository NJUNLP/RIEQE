...
ERROR_SENT_NUM = int(TOTAL_SAMPLES * ERROR_RATIO)   # 8000
NOERROR_SENT_NUM = TOTAL_SAMPLES - ERROR_SENT_NUM   # 2000

# ERROR_SENT_NUM = 10000
# NOERROR_SENT_NUM = 8000

OUTPUT_PATH = f"data/unitErrorDetection_LF_nonCOT_10k-{lang_pair}.json"
...
with open(ERROR_PATH, "r", encoding="utf-8") as f:
    error_sentences = json.load(f)[:ERROR_SENT_NUM]

with open(NOERROR_PATH, "r", encoding="utf-8") as f:
    noerror_sentences = json.load(f)[:NOERROR_SENT_NUM]
...
print("\n✅ DONE")
print(f"Sentence-level samples: {TOTAL_SAMPLES}")
print(f"Error units: {len(error_units)}")
print(f"No-error units kept: {KEEP_NOERROR}")
