...
TOTAL_SAMPLES = 20000
ERROR_RATIO = 0.5
ERROR_NUM = int(TOTAL_SAMPLES * ERROR_RATIO)
NOERROR_NUM = TOTAL_SAMPLES - ERROR_NUM

# ERROR_NUM = 10000
# NOERROR_NUM = 8000

output_path = f"data/sequentialErrorDetection_LF_nonCOT_10k-{lang_pair}.json"
...
error_samples = error_data[:ERROR_NUM]
noerror_samples = noerror_data[:NOERROR_NUM]

data = error_samples + noerror_samples
random.shuffle(data)

print(f"Loaded {len(error_samples)} error + {len(noerror_samples)} no-error samples")
